...
*  3- Gerar o código proveniente dessas configurações;
*  4- Codar nas linhas certas, para que o compilador considere o que foi modificado.
*/
...
int main(void)
{
  ...
  /* USER CODE BEGIN 1 */
	gpio_state_t btnState = gpio_high_level;
	gpio_state_t blinkState = gpio_high_level;
	gpio_pin_t LED = {.pin = LED_Pin, .port = LED_GPIO_Port};
	gpio_pin_t LD3 = {.pin = LD3_Pin, .port = LD3_GPIO_Port};
	gpio_pin_t KEY = {.pin = KEY_Pin, .port = KEY_GPIO_Port};
  /* USER CODE END 1 */
  ...
  while (1)
  {
	  if(gpio_read(KEY) == gpio_low_level
			  && btnState == gpio_high_level ){
		  blinkState = !blinkState;
	  }

	  btnState = gpio_read(KEY);
	  delay_ms(50);
